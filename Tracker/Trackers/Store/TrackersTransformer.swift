...
@objc(TrackersTransformer)
final class TrackersTransformer: ValueTransformer {
    ...
    override func transformedValue(_ value: Any?) -> Any? {
        guard let trackersArray = value as? [Tracker] else { return nil }
        do {
            // Преобразуем массив строк в Data
            let data = try NSKeyedArchiver.archivedData(withRootObject: trackersArray, requiringSecureCoding: true)
